...
const Labels = ({ task, onToggleLabel, onClose }) => {
  ...
  return (
    <div className="absolute top-0 right-0 ml-2 w-72 bg-white rounded-lg shadow-xl border border-gray-200 z-50 p-3 animate-in fade-in zoom-in duration-150">
      <LabelHeader
        title={isEditorOpen ? (editingLabelId ? "Sửa nhãn" : "Tạo nhãn mới") : "Nhãn dán"}
      ...
      {isEditorOpen ? (
        ...
        <div>
          <p className="text-[11px] font-bold text-gray-500 uppercase mb-2">Nhãn dán của bảng</p>
          <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1 custom-scrollbar">
            {allLabels.length === 0 ? (
              <div className="text-center py-1 text-gray-400 text-sm italic">Chưa có nhãn nào...</div>
            ) : (
              allLabels.map(label => (
                <LabelItem
